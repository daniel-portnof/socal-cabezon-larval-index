...
```{r preferred model notation}
# One-time setup if you don't have tinytex
install.packages("tinytex")
tinytex::install_tinytex()  # installs a minimal LaTeX distribution

# Then for the equations:
library(tinytex)

# Write a standalone .tex file
tex_source <- r"(
\documentclass[border=10pt]{standalone}
\usepackage{amsmath}
\begin{document}
$\begin{aligned}
\textrm{logit}(p_{st}) &= X_{st}\beta^{(1)} + \omega_s^{(1)} + \varepsilon_{st}^{(1)} \\
\ln(\mu_{st}) &= X_{st}\beta^{(2)} + \omega_s^{(2)} \\
\omega_s^{(k)} &\sim \textrm{MVNormal}\left(0, \Sigma_{\omega^{(k)}}\right), \quad k \in \{1,2\} \\
\varepsilon_{st}^{(1)} &\sim \textrm{MVNormal}\left(0, \Sigma_{\varepsilon^{(1)}}\right)
\end{aligned}$
\end{document}
)"

writeLines(tex_source, "model_eq.tex")
pdflatex("model_eq.tex")  # produces model_eq.pdf
