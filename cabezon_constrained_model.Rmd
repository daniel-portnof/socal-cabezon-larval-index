...
AIC_table <- data.frame(
  ...
  model = c(
    "Delta-Lognormal: Global Intercept, spatial on, spatiotemporal off",
    "Delta-Lognormal Global Intercept + spatial + spatiotemporal delta AR1 + spatiotemporal lognormal AR1 (Spawning Season)",
    "Delta-Lognormal Global Intercept + spatial + spatiotemporal delta AR1 + spatiotemporal lognormal iid (Spawning Season)",
    "Delta-Lognormal Global Intercept + spatial + spatiotemporal off (Spawning Season)",
    "Delta-Lognormal fixed year effect + spatial + spatiotemporal off (Spawning Season)",
    "Delta-Lognormal fixed year effect + spatial + spatiotemporal delta iid + spatiotemporal lognormal iid (Spawning Season)",
    "Delta-Lognormal fixed year effect + spatial + spatiotemporal delta iid + spatiotemporal lognormal off (Spawning Season)",
    "Delta-Lognormal Global Intercept + spatial + spatiotemporal delta AR1 + spatiotemporal lognormal off (Spawning Season)"
  ),
  AIC = c(
    AIC(ss_cab_fit_dln0),
    AIC(ss_cab_fit_dln1),
...
cog <- get_cog(ss_dln3_cab_map, format = "wide", bias_correct = FALSE)
...
ccf_ssb    <- ccf_boot(ss_dln3_combined_obs$larvae_index,
                       ...
                       lag.max = 10, plot = "Spearman", B = 1000)
# Swap: LOESS-detrended instead of first-differenced
ccf_dssb   <- ccf_boot(loess_df$resid_index,
                       loess_df$resid_ssb,
                       lag.max = 10, plot = "Spearman", B = 1000)
dev.off()
...
ccf_df <- bind_rows(
  ...
  tidy_ccf(ccf_dssb,   "SSB (LOESS-detrended)")
)
...
p_year <- ggplot(ss_plot_data_year, aes(x = year, y = count, fill = tow_type)) +
  ...
  labs(
    title = "Cabezon Larvae: Positive vs. Total Tows",
    subtitle = "Annual sampling effort, spawning season (Jan - Mar) 1981–2015",
    y = "Number of Tows"
  ) +
  shared_bar_theme
...
obs_aggregate <- obs_plot %>%
  ...
  summarise(
    ...
    n_visits = n(),
    n_positive = sum(larvae_positive, na.rm = TRUE),
    mean_abundance = mean(larvae_100m3, na.rm = TRUE),
    max_abundance = max(larvae_100m3, na.rm = TRUE),
    ever_positive = any(larvae_positive, na.rm = TRUE),
    .groups = "drop"
  )

# Compute padded plot limits (10% buffer on each side)
...
ggplot() +
  ...
  geom_point(
    data = obs_aggregate %>% filter(ever_positive),
    aes(x = X_m, y = Y_m, size = mean_abundance),
    alpha = 0.7,
    color = "#2C5F8D"
  ) +
  coord_sf(
  ...
  labs(
    title = "Observed cabezon larval abundance by CalCOFI station",
    subtitle = "Aggregated across all sampling years",
    x = NULL,
    y = NULL
  ) +
  theme_minimal()
...
p_effort <- ggplot() +
  ...
  labs(
    title = "CalCOFI sampling effort by station",
    subtitle = "Total tows aggregated across all sampling years, 1981–2015",
    x = NULL,
    y = NULL
  ) +
  theme_minimal()
...
ggplot(ss_dln3_cab_index, aes(x = year, y = est)) +
  ...
  scale_y_continuous(labels = comma, 
                     ...
                     expand = expansion(mult = c(0, 0.05))) +
  labs(
    title = "Preferred Cabezon larval abundance index, 1981–2015",
...
```{r AIC comparison table}
library(dplyr)
library(tibble)
library(knitr)
library(kableExtra)

# ---- Build a clean, structured AIC table ----
# Define model components explicitly rather than parsing the description strings.
# This is cleaner, less error-prone, and easier to maintain when you add models.

aic_table_clean <- tibble::tribble(
  ~fit_name,         ~fixed,             ~spatial, ~st_enc, ~st_pos,  ~AIC,
  "ss_cab_fit_dln0", "Intercept",        "On",     "Off",   "Off",    AIC(ss_cab_fit_dln0),
  "ss_cab_fit_dln1", "Intercept",        "On",     "AR1",   "AR1",    AIC(ss_cab_fit_dln1),
  "ss_cab_fit_dln2", "Intercept",        "On",     "AR1",   "IID",    AIC(ss_cab_fit_dln2),
  "ss_cab_fit_dln3", "Intercept",        "On",     "IID",   "Off",    AIC(ss_cab_fit_dln3),
  "ss_cab_fit_dln4", "Year (fixed)",     "On",     "Off",   "Off",    AIC(ss_cab_fit_dln4),
  "ss_cab_fit_dln5", "Year (fixed)",     "On",     "IID",   "IID",    AIC(ss_cab_fit_dln5),
  "ss_cab_fit_dln6", "Year (fixed)",     "On",     "IID",   "Off",    AIC(ss_cab_fit_dln6),
  "ss_cab_fit_dln7", "Intercept",        "On",     "AR1",   "Off",    AIC(ss_cab_fit_dln7)
) %>%
  mutate(
    delta_AIC = AIC - min(AIC),
    weight = exp(-0.5 * delta_AIC) / sum(exp(-0.5 * delta_AIC)),
    model_id = sub("ss_cab_fit_", "M", fit_name)  # cleaner row labels: M dln0, M dln1, ...
  ) %>%
  arrange(delta_AIC) %>%
  select(model_id, fixed, spatial, st_enc, st_pos, AIC, delta_AIC, weight)
...
aic_table_clean %>%
  ...
  kable(
    format = "latex",
    booktabs = TRUE,
    col.names = c("Model", "Fixed effects", "Spatial", 
                  "ST (encounter)", "ST (positive)", 
                  "AIC", "$\\Delta$AIC", "$w_i$"),
    align = c("l", "l", "c", "c", "c", "r", "r", "r"),
    caption = paste(
      "Model selection results for delta-lognormal species distribution models",
      "of cabezon (\\textit{Scorpaenichthys marmoratus}) larvae in the Southern",
      "California Bight, January--March, ranked by AIC. ST = spatiotemporal",
      "random field; AR1 = first-order autoregressive; IID = independent and",
      "identically distributed across years; $w_i$ = AIC model weight."
    ),
    label = "tab:aic-cabezon-dln",
    escape = FALSE
  ) %>%
  kable_styling(
    latex_options = c("booktabs", "hold_position"),
    font_size = 9,
    position = "center"
  ) %>%
  row_spec(0, bold = TRUE) %>%
  # Optionally highlight the top model
  row_spec(1, bold = TRUE)

# Print to a .tex file you can \input{} into your manuscript
aic_latex <- aic_table_clean %>%
  mutate(
    AIC = sprintf("%.1f", AIC),
    delta_AIC = sprintf("%.2f", delta_AIC),
    weight = sprintf("%.3f", weight)
...
writeLines(as.character(aic_latex), "outputs/aic_table.tex")
```
