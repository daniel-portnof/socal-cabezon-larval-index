...
ss_cab_fit_dln1 <- ss_cab_fit_dln3
```
...
# ccf_boot prints a plot AND returns a data frame invisibly.
# Suppress printed plots by routing to a null PDF device.
...
# Keep Spearman columns only; flag significance (point falls outside CI band).
...
ccf_df <- bind_rows(
  ...
  tidy_ccf(ccf_ssb,    "SSB"),
  tidy_ccf(ccf_dssb,   "\u0394 Larval index vs. \u0394 SSB")
)

ccf_df$panel <- factor(
  ccf_df$panel,
  levels = c("Age-0 recruits", "Recruitment deviations",
             "SSB", "\u0394 Larval index vs. \u0394 SSB")
)
...
p_ccf <- ggplot(ccf_df, aes(x = lag, y = r)) +
  ...
  labs(
    ...
    subtitle = "Delta-lognormal fixed-year model; blue band = bootstrap 95% CI (B = 1000)"
  ) +
...
stations <- st_read(kml_path, quiet = TRUE) %>%
  ...
  filter(line %in% core_lines, stn >= 27, stn <= 120) %>%
  st_transform(utm_crs)
...
# ---- 4. Seaward-only shelf study border -------------------------------

shelf <- stations %>%
  filter(stn <= shelf_station_max)

hull <- st_concave_hull(st_union(shelf), ratio = concave_ratio)

land_union <- ca_coast %>%
  st_make_valid() %>%
  st_union()

# Remove land from the study-region hull
region_poly <- st_difference(hull, land_union)

# Take boundary of the ocean-only polygon
region_bdry <- st_boundary(region_poly)

# Remove boundary pieces close to land/coast
region_sea <- st_difference(
  region_bdry,
  st_buffer(land_union, coast_clip_km * 1000)
)

# Clean geometry collection, keep only meaningful line pieces
region_sea <- st_collection_extract(region_sea, "LINESTRING") %>%
  st_sf(geometry = ., crs = st_crs(utm_crs)) %>%
  mutate(length_km = as.numeric(st_length(geometry)) / 1000) %>%
  filter(length_km >= min_border_segment_km)

# ---- 4b. Remove lingering eastern border fragments --------------------

# First keep only line geometries
region_sea <- st_collection_extract(region_sea, "LINESTRING")

# Remove tiny fragments
region_sea <- st_sf(geometry = region_sea, crs = st_crs(utm_crs)) %>%
  mutate(length_km = as.numeric(st_length(geometry)) / 1000) %>%
  filter(length_km >= 20)

# Then explicitly clip away the far-eastern remnants
# Move this longitude west/east if needed
east_cutoff_lon <- -117.55

west_mask_ll <- st_as_sfc(
  st_bbox(
    c(
      xmin = -126,
      xmax = east_cutoff_lon,
      ymin = 28.5,
      ymax = 36.5
    ),
    crs = 4326
...
region_sea <- region_sea %>%
  st_transform(4326) %>%
  st_intersection(west_mask_ll) %>%
  st_transform(utm_crs)

# ---- 5. Zoomed-out limits ---------------------------------------------

bb   <- st_bbox(stations)
xpad <- (bb["xmax"] - bb["xmin"]) * zoom_pad
ypad <- (bb["ymax"] - bb["ymin"]) * zoom_pad

xlim <- c(bb["xmin"] - xpad, bb["xmax"] + xpad)
ylim <- c(bb["ymin"] - ypad, bb["ymax"] + ypad)

# ---- 6. Labels ---------------------------------------------------------
# Adjust lon/lat here if you want to move labels manually.

california_ll <- tibble(
  label = "California",
  lon = -118.05,
  lat = 35.05
)
...
california_sf <- st_as_sf(california_ll, coords = c("lon", "lat"), crs = 4326) %>%
  st_transform(utm_crs)

mexico_sf <- st_as_sf(mexico_ll, coords = c("lon", "lat"), crs = 4326) %>%
  st_transform(utm_crs)

california_xy <- bind_cols(
  st_drop_geometry(california_sf),
  as_tibble(st_coordinates(california_sf)) %>%
    rename(X_m = X, Y_m = Y)
)

mexico_xy <- bind_cols(
  st_drop_geometry(mexico_sf),
  as_tibble(st_coordinates(mexico_sf)) %>%
    rename(X_m = X, Y_m = Y)
)
...
p_grid <- ggplot() +
  ...
  # CalCOFI station markers: increase size here
  geom_sf(
    data = stations,
    inherit.aes = FALSE,
    shape = 21,
  ...
  # Abundance bubble size: increase range here
  scale_size_continuous(
    name = "Mean larvae / 100 m³",
    range = c(2, 10)
  ) +
  ...
  labs(
    title = "Cabezon larval abundance across the CalCOFI core grid",
    subtitle = "Aggregated across all sampling years; shelf study region outlined",
    x = NULL,
    y = NULL
  ) +
